...
# 화면 상태 문구
current_status = "Waiting for sensor data"
last_event_name = "None"
last_event_details = "No completed event yet"
last_event_time_text = "-"
...
def classify_current_impact_sequence():
    ...
    global last_event_name
    global last_event_details
    global last_event_time_text
    ...
    if hit_count == 1:
        ...
        current_status = "Single impact candidate"
        last_event_name = "Single impact candidate"
        last_event_details = f"1 isolated hit | Score {hit_values[0]:.1f}"
        last_event_time_text = datetime.fromtimestamp(event_time).strftime("%H:%M:%S")
    ...
    if hit_count == SOS_REQUIRED_HITS:
        ...
        if is_sos:
            ...
            current_status = "SOS CONFIRMED"
            last_event_name = "SOS confirmed"
            last_event_details = (
                f"IOI {iois[0]:.3f}s / {iois[1]:.3f}s | "
                f"Error {ioi_error_ratio:.2f} | AC {autocorr_score:.2f}"
            )
            last_event_time_text = datetime.fromtimestamp(event_time).strftime("%H:%M:%S")
        ...
        current_status = "Irregular three-hit sequence"
        ...
        last_event_name = "Rejected three-hit sequence"
        last_event_details = (
            f"IOI {iois[0]:.3f}s / {iois[1]:.3f}s | "
            f"Error {ioi_error_ratio:.2f} | AC {autocorr_score:.2f}"
        )
    ...
    current_status = f"Other repeated vibration ({hit_count} hits)"
    last_event_name = "Other repeated vibration"
    last_event_details = (
        f"{hit_count} hits | Duration "
        f"{hit_times[-1] - hit_times[0]:.3f}s"
    )
    last_event_time_text = datetime.fromtimestamp(event_time).strftime("%H:%M:%S")
    ...
    if is_above:
        if not in_peak:
            if t - last_peak_time >= MIN_PEAK_DISTANCE_SEC:
                ...
                current_status = "Impact candidate detected"
        else:
        ...
        if in_peak:
            ...
            is_peak = 1
            current_status = (
                f"Sequence in progress: "
                f"{len(impact_sequence_times)} / {SOS_REQUIRED_HITS} hits"
            )
...
# All plot text is English to avoid missing Korean glyphs.
# =========================
...
# Upper-left: final detection score
ax_detection = fig.add_subplot(grid[0, 0])

# Upper-right: text-only live status dashboard
ax_status = fig.add_subplot(grid[0, 1])

# Lower-left: preprocessing result
ax_signal = fig.add_subplot(grid[1, 0])

# Lower-right: current three-hit sequence
ax_sequence = fig.add_subplot(grid[1, 1])
...
line_boosted, = ax_detection.plot(
    [],
    [],
    linewidth=1.5,
    label="Boosted score",
...
line_raw_score, = ax_detection.plot(
    [],
    [],
    linewidth=1.0,
    alpha=0.40,
...
line_dynamic_th, = ax_detection.plot(
    [],
    [],
    linewidth=1.4,
    linestyle="--",
...
peak_scatter = ax_detection.scatter(
    [],
    [],
    marker="o",
    s=28,
    alpha=0.65,
    label="Detected impact",
)

single_scatter = ax_detection.scatter(
    [],
    [],
    marker="^",
    s=85,
...
other_scatter = ax_detection.scatter(
    [],
    [],
    marker="s",
    s=65,
...
sos_scatter = ax_detection.scatter(
    [],
    [],
    marker="*",
    s=220,
...
ax_detection.set_xlabel("Time relative to now (s)")
ax_detection.set_ylabel("Vibration score")
ax_detection.set_xlim(-PLOT_SEC, 0)
ax_detection.set_ylim(0, Y_MAX_SCORE)
ax_detection.grid(True, alpha=0.28)
ax_detection.legend(
    loc="upper left",
    ncol=2,
    frameon=True,
...
# ------------------------------------------------------------
# Panel 2: Live status dashboard
# ------------------------------------------------------------

ax_status.set_title(
    "2. Live Decision Status",
    loc="left",
    fontweight="bold",
)
ax_status.axis("off")
...
criteria_text = ax_status.text(
    0.03,
    0.025,
    (
        f"SOS criteria: {SOS_REQUIRED_HITS} hits | "
        f"IOI {SOS_MIN_IOI_SEC:.2f}–{SOS_MAX_IOI_SEC:.2f}s | "
        f"Duration ≤ {SOS_PATTERN_MAX_SEC:.2f}s\n"
        f"IOI error ≤ {SOS_MAX_IOI_ERROR_RATIO:.2f} | "
        f"Autocorrelation ≥ {MIN_AUTOCORR_SCORE:.2f}"
    ),
    transform=ax_status.transAxes,
    va="bottom",
    ha="left",
...
# ------------------------------------------------------------
# Panel 3: Processed vibration signal
# ------------------------------------------------------------

line_vib, = ax_signal.plot(
    [],
    [],
    linewidth=1.0,
    alpha=0.60,
    label="Absolute vibration",
)

line_envelope, = ax_signal.plot(
    [],
    [],
    linewidth=1.5,
    label="Envelope",
...
ax_signal.set_title(
    "3. Noise Removal and Envelope",
    loc="left",
    fontweight="bold",
)
ax_signal.set_xlabel("Time relative to now (s)")
ax_signal.set_ylabel("Acceleration (g)")
ax_signal.set_xlim(-PLOT_SEC, 0)
ax_signal.set_ylim(0, max(MIN_SIGNAL_G * 4, 0.01))
ax_signal.grid(True, alpha=0.28)
ax_signal.legend(
    loc="upper left",
    ncol=2,
    frameon=True,
...
# ------------------------------------------------------------
# Panel 4: Current three-hit sequence
# ------------------------------------------------------------

sequence_line, = ax_sequence.plot(
    [],
    [],
    linewidth=1.4,
    marker="o",
...
sequence_scatter = ax_sequence.scatter(
    [],
    [],
    marker="o",
    s=100,
...
sequence_hint_text = ax_sequence.text(
    0.03,
    0.96,
...
ax_sequence.set_title(
    "4. Current Three-Hit Pattern",
    loc="left",
    fontweight="bold",
)
ax_sequence.set_xlabel("Time from first hit (s)")
ax_sequence.set_ylabel("Peak score")
ax_sequence.set_xlim(-0.12, SOS_PATTERN_MAX_SEC + 0.25)
ax_sequence.set_ylim(0, Y_MAX_SCORE)
ax_sequence.grid(True, alpha=0.28)
ax_sequence.legend(
    loc="lower right",
    frameon=True,
)
...
fig.suptitle(
    "Realtime Floor-Vibration SOS Monitor",
    fontsize=17,
    fontweight="bold",
    y=0.98,
)
...
fig.text(
    0.5,
    0.945,
    "Dynamic threshold  |  Peak detection  |  IOI regularity  |  Autocorrelation",
    ha="center",
    fontsize=10,
)
...
def remove_sequence_annotations():
    """
    Remove only Hit and IOI annotations from the previous frame.
    """
    for artist in list(ax_sequence.texts):
        if artist is sequence_hint_text:
            continue
        artist.remove()
...
def update_status_panel():
    latest_score = (
        float(boosted_scores[-1])
        if boosted_scores
        else 0.0
    )
    latest_threshold = (
        float(dynamic_thresholds[-1])
    ...
    if impact_sequence_times:
        if len(impact_sequence_times) >= 2:
            active_iois = np.diff(
                np.asarray(impact_sequence_times, dtype=float)
            )
            active_ioi_text = " / ".join(
                f"{value:.3f}s"
                for value in active_iois
            )
        else:
            active_ioi_text = "Waiting for hit 2"
    else:
        active_ioi_text = "No active sequence"

    status_text.set_text(
        f"CURRENT STATUS\n"
        f"{current_status}\n\n"
        f"Hits              {len(impact_sequence_times)} / {SOS_REQUIRED_HITS}\n"
        f"IOI               {active_ioi_text}\n"
        f"Score / threshold {latest_score:.2f} / {latest_threshold:.2f}\n"
        f"Noise             {latest_noise:.5f} g\n\n"
        f"LAST EVENT\n"
        f"{last_event_name}  ({last_event_time_text})\n"
        f"{last_event_details}"
    )


def update_sequence_panel():
    remove_sequence_annotations()

    if impact_sequence_times:
        first_hit_time = impact_sequence_times[0]

        sequence_x = [
            hit_time - first_hit_time
            for hit_time in impact_sequence_times
        ]
        sequence_y = list(impact_sequence_values)

        sequence_line.set_data(sequence_x, sequence_y)
        set_scatter_data(
            sequence_scatter,
            sequence_x,
            sequence_y,
        ...
        if len(sequence_x) >= 2:
            iois = np.diff(np.asarray(sequence_x, dtype=float))

            for index in range(len(sequence_x) - 1):
                left_x = sequence_x[index]
                right_x = sequence_x[index + 1]
                ...
                ax_sequence.annotate(
                    f"IOI {index + 1}: {iois[index]:.3f}s",
                    xy=(mid_x, label_y),
                    ha="center",
                    fontsize=9,
                )

            sequence_hint_text.set_text(
                "Detected IOI: "
                + " / ".join(
                    f"{value:.3f}s"
                    for value in iois
            ...
            sequence_hint_text.set_text(
                "Hit 1 detected. Waiting for hit 2"
            )

    else:
        sequence_line.set_data([], [])
        set_scatter_data(sequence_scatter, [], [])
        sequence_hint_text.set_text(
            "Waiting for the first hit"
        )
...
def update_plot(frame):
    ...
    vibration_y = list(vibs)
    envelope_y = list(envelopes)
    noise_y = list(noise_levels)
    ...
    single_x = [
        pt - t0
        for pt in single_impact_times
    ]
    single_y = list(single_impact_values)

    other_x = [
        pt - t0
        for pt in other_event_times
    ]
    ...
    line_vib.set_data(xs, vibration_y)
    line_envelope.set_data(xs, envelope_y)
    line_noise.set_data(xs, noise_y)
    ...
    # Keep the main score scale fixed for easier comparison.
    ax_detection.set_xlim(-PLOT_SEC, 0)
    ax_detection.set_ylim(0, Y_MAX_SCORE)

    # Scale only the signal panel because g values depend on the floor.
    signal_values = (
        vibration_y
        + envelope_y
        + noise_y
    ...
    if finite_signal_values:
        signal_upper = max(finite_signal_values) * 1.20
        signal_upper = max(
            signal_upper,
            MIN_SIGNAL_G * 4,
            0.01,
        )
        ax_signal.set_ylim(0, signal_upper)
