...
DEVICE_ADDRESS = "DD:D6:0F:01:23:A5" # 기기 번호임

CSV_PATH = "imu_realtime_peak_boosted.csv"
...
# 평소 노이즈보다 몇 배 이상 튀면 peak로 볼지
# boosted_score 기준
SCORE_THRESHOLD = 30.0

# score 증폭 강도
# 클수록 평소보다 튄 값이 더 크게 보임
...
def process_sample(sample, writer):
    ...
    # peak 판정
    is_above = boosted_score > SCORE_THRESHOLD and envelope > MIN_SIGNAL_G
    is_peak = 0
...
line_th, = ax.plot([], [], linestyle="--", label="score threshold")
peak_scatter = ax.scatter([], [], marker="o", label="peak")

ax.set_title("WT901BLECL Realtime Boosted Peak Detection")
...
def update_plot(frame):
    ...
    if not times:
        return line_boosted, line_raw_score, line_th, peak_scatter
